    ...
    for alpha in np.arange(start_alpha, 1.1, 0.1):
        for beta in np.arange(start_beta, 1.1, 0.1):
            for gamma in np.arange(start_gamma, 1.1, 0.1):
                    distillation = distiller(teacher_model_name, student_model_name, val_records_path, True)
                    distill_data_path = distillation.distilled_model_path + "/" + distillation.distilled_model_name + "/distillation_data.csv"
