...
class distiller():
    ...
    def log(self,message):
        if self.save_log == True:
            log = open(f"logs/distillation/{self.log_name}", "a")
            log.write(message)
            log.close()
        #pass
    ...
    def attention_transfer(self, attention_loss):
        if "ssd_mobilenet_v1_fpn_640x640_coco17_tpu-8" in self.student_model_name and "ssd_resnet152_v1_fpn_640x640_coco17_tpu-8" in self.teacher_model_name:
            loss = 0
            losses = tf.stack([attention_loss(self.teacher_model.variables[i], self.student_model.variables[i]) for i in range(92, 98)] + \
                            [attention_loss(self.teacher_model.variables[563 + i], tf.tile(self.student_model.variables[179 + i], [1,1,2,1])) if (i == 0 or i == 2 or i == 4) else attention_loss(self.teacher_model.variables[563 + i], self.student_model.variables[179 + i]) for i in range(12)] + \
                            [attention_loss(self.teacher_model.variables[655 + i], self.student_model.variables[271 + i]) for i in range(4)] + \
                            [attention_loss(self.teacher_model.variables[969 + i], self.student_model.variables[329 + i]) for i in range(4)])
    ...
    def distill(self, epoch_num, optimizer, distillation_loss_fn, attention_loss_fn, save_every_n_epochs=10):
        ...
        for epoch in range(start_step, epoch_num):
            ...
            end_time = time.time()
            all_epoch_losses.append([self.log_name, end_time - start_time, optimizer.learning_rate.numpy(), losses_dict["distill_avg"], losses_dict["localization_avg"], losses_dict["classification_avg"], losses_dict["attention_avg"], losses_dict["total_avg"]])
            ...
            self.print_message(" - | - Distillation loss: {:} - Classification loss: {:} - Localization loss: {:} - Attention loss: {:} - | - Total loss: {:} \n".format(losses_dict["distill"][-1], losses_dict["classification"][-1], losses_dict["localization"][-1], losses_dict["attention"][-1], losses_dict["total"][-1]))
    ...
    def distillation_init(self, epoch, alpha_n = 1, beta_n = 1, gamma_n = 1, delta_n = 0, temperature_n = 10, learning_rate_n = 0.001):
        ...
        losses_names = ['Log Name', "Time", 'Learning Rate', 'Distillation Loss', 'Localization Loss', 'Classification Loss', "Attention Loss", 'Total Loss']
